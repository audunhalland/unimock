...
mod should_print_pattern_mismatch_on_call_order_failure {
    use super::*;
...
mod should_print_eq_mismatch_on_call_order_failure {
    use super::*;
...
mod should_print_message_about_missing_debug {
    use super::*;
...
mod should_print_message_about_failed_inequality_check {
    use super::*;
    ...
    #[test]
    #[should_panic(
        expected = "Trait::foo(S { value: \"a\" }): Method invoked in the correct order (1), but inputs didn't match Trait::foo(ne!(..)) at tests/it/pretty_mismatches.rs:106. \nInequality mismatch for input #0:\nS { value: \"a\" }"
    )]
    fn test() {
        let u = Unimock::new(
            TraitMock::foo
...
mod should_complain_about_missing_debug_representation_for_inequality_mismatch {
    use super::*;
    ...
    #[test]
    #[should_panic(
        expected = "Trait::foo(?): Method invoked in the correct order (1), but inputs didn't match Trait::foo(ne!(..)) at tests/it/pretty_mismatches.rs:138. \nInequality mismatch for input #0:\nActual value unexpectedly equalled expected value, but can't display diagnostics because the type is likely missing #[derive(Debug)]."
    )]
    fn test() {
        let u = Unimock::new(
            TraitMock::foo
...
mod should_print_all_mismatches_on_matched_function {
    use super::*;
